...
const OrderHistory = ({ customerId, customerName }) => {
  ...
  const translateProductionStatus = (status) => {
    switch(status?.toLowerCase()) {
      case 'zakończone':
        return 'Completed';
      case 'w trakcie':
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-3 flex-shrink-0">
                    ...
                    >
                      ...
                      <span className="ml-1 hidden sm:inline">{translateStatus(order.status)}</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {order.paymentStatus && (
                    <div className="col-span-2">
                      ...
                      <div className="text-white">{translatePaymentStatus(order.paymentStatus)} {order.paymentMethod && `(${translatePaymentMethod(order.paymentMethod)})`}</div>
                    </div>
                  )}
              ...
              <div className="hidden lg:block p-6">
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    {order.paymentStatus && (
                      <p className="text-sm text-gray-400">
                        Payment: {translatePaymentStatus(order.paymentStatus)} {order.paymentMethod && `(${translatePaymentMethod(order.paymentMethod)})`}
                      </p>
                    )}
                    {order.shippingAddress && (
                        ...
                        <div className="text-xs text-gray-400 space-y-1">
                          ...
                          {item.productionStatus && (
                            <p className={getProductionStatusColor(item.productionStatus)}>
                              Production Status: {translateProductionStatus(item.productionStatus)}
                            </p>
                          )}
                          {item.unit && (
                            <p>Unit: {item.unit}</p>

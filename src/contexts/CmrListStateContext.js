...
const { Context, Provider, useListState } = createListStateContext({
  ...
  reducerCases: (state, action) => {
    switch (action.type) {
      ...
      case actionTypes.SET_ITEM_FILTER:
        return { ...state, itemFilter: action.payload, page: 1, lastUpdated: Date.now() };
  ...
  createActions: (dispatch) => ({
    ...
    setItemFilter: (filter) => dispatch({ type: actionTypes.SET_ITEM_FILTER, payload: filter }),
...
export const CmrListStateProvider = Provider;
export const useCmrListState = useListState;

/**
 * Weekly Consumption Analysis Report - Scheduled Function
 * Generuje cotygodniowy raport analizy konsumpcji MO z wykorzystaniem Gemini AI
 * Uruchamiana raz w tygodniu w niedzielę o 06:00
 *
 * Zapisuje raport do: reports/weeklyConsumptionAnalysis
 *
 * Analizowane elementy:
 * - Konsumpcje MO (consumedMaterials, actualMaterialUsage)
 * - Historia stanów magazynowych (transakcje ISSUE/RECEIVE)
 * - Obecne stany magazynowe (inventory)
 * - Niewykorzystane pozostałości partii
 * - Błędy konsumpcji (brak konsumpcji, nieużyte rezerwacje)
 * - Odchylenia plan vs realizacja
 * - Partie zamrożone (z rezerwacjami ale bez konsumpcji)
 * - Partie bez ruchu (przez miesiąc)
 * - Wydajność produkcji
 * - Top konsumowane materiały
 *
 * DEPLOYMENT:
 * firebase deploy --only functions:bgw-mrp:generateWeeklyConsumptionReport
 */
...
const generateWeeklyConsumptionReport = onSchedule(
    {
      schedule: "0 6 * * 0", // Niedziela o 06:00
    ...
    async (event) => {
      logger.info(
          "generateWeeklyConsumptionReport - rozpoczynam generowanie raportu",
      );
      ...
      try {
        ...
        const [
          ...
          // Partie z niską ilością (pozostałości)
          db
          ...
          // Wszystkie aktywne partie (dla analizy zamrożonych)
        ...
        logger.error("generateWeeklyConsumptionReport - błąd", {
          error: error.message,
        });

        // Zapisz błąd w raporcie
...
function performAdvancedAnalysis(data) {
  ...
  // ============================================================
  // ANALIZA ZADAŃ - PLAN VS REALIZACJA
  // ============================================================
  const materialConsumptionMap = new Map(); // Dla top materiałów
  ...
  tasks.forEach((task) => {
    ...
    // ============================================================
    // PORÓWNANIE PLANOWANEJ VS RZECZYWISTEJ KONSUMPCJI
    // ============================================================
    if (task.materials && task.materials.length > 0) {
      task.materials.forEach((material) => {
        ...
        // Rzeczywista ilość (z actualMaterialUsage lub consumedMaterials)
        let actualQty = 0;
        if (
    ...
    // ============================================================
    // WYKRYWANIE BŁĘDÓW KONSUMPCJI
    // ============================================================

    // Błąd: Zadanie zakończone bez konsumpcji (ale ma materiały)
    if (
      isCompleted &&
      !hasConsumption &&
    ...
    // Błąd: Rezerwacje bez konsumpcji lub częściowo niewykorzystane
  ...
  // ============================================================
  // PARTIE "ZAMROŻONE" (z rezerwacjami ale bez ostatniej konsumpcji)
  // ============================================================
  ...
  statistics.uniqueMaterialsConsumed =
    statistics.uniqueMaterialsConsumed.size;
...
module.exports = {generateWeeklyConsumptionReport};
